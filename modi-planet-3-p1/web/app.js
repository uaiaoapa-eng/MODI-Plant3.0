(() => {
  ...
  const ELEMENTARY_LESSONS = [
    ...
    ["모터와 움직임", "MODI"],
    ["데이터 이용하기", "Web"],
    ["Web에서 MODI 제어", "Web+MODI"],
    ["스마트 장치 만들기", "Web+MODI"],
    ["나만의 프로젝트", "Web+MODI"]
  ];

  const PLACEHOLDER_LESSONS = Array.from({ length: 9 }, (_, index) => [
    String(index + 1) + "차시 교육 콘텐츠",
    index < 3 ? "Web" : (index < 6 ? "MODI" : "Web+MODI")
  ]);
  ...
  function renderHome() {
    return [
      ...
      '<p class="eyebrow">MODI PLANET 3.0</p>',
      '<h1 id="homeTitle">오늘 무엇을<br><span class="accent-word">해볼까요?</span></h1>',
      '<p>수업 흐름을 따라 차근차근 배우거나, AI와 이야기하며 나만의 작품을 자유롭게 만들어보세요.</p>',
      "</header>",
  ...
  function renderLearn() {
    ...
    return [
      ...
      '<span class="page-badge">교육과정 선택</span></div>',
      '<header class="page-heading">',
      '<p class="step-kicker">Learn</p>',
      '<h1 id="learnTitle">나에게 맞는 과정을<br>선택해보세요.</h1>',
      "<p>학교급에 맞춘 9개의 프로젝트 수업이 준비될 예정이에요. P1에서는 전체 과정의 구조를 먼저 살펴볼 수 있어요.</p>",
      "</header>",
      '<div class="grade-grid" aria-label="학교급 선택">',
      GRADE_BANDS.map((grade) => [
        '<button class="grade-card" type="button" data-grade="',
        escapeHtml(grade.id),
        '"><span class="grade-count">9차시</span><span class="grade-icon">',
        grade.icon,
        "</span><h2>",
        escapeHtml(grade.label),
        "</h2><p>",
        escapeHtml(grade.subtitle),
        "</p></button>"
      ].join("")).join(""),
      "</div>",
      "</div>",
  ...
  function renderCurriculumShell() {
    const grade = getGradeBand(state.learnGrade);
    const lessons = grade.id === "elementary" ? ELEMENTARY_LESSONS : PLACEHOLDER_LESSONS;

    return [
      '<section class="page-view learn-view" aria-labelledby="curriculumTitle">',
      '<div class="page-frame">',
      '<div class="page-toolbar"><button class="back-button" type="button" data-action="all-grades">',
      ICONS.back, "<span>학교급 다시 선택</span></button>",
      '<span class="page-badge">9차시 커리큘럼</span></div>',
      '<header class="curriculum-header"><div>',
      '<p class="step-kicker">', escapeHtml(grade.label), " 과정</p>",
      '<h1 id="curriculumTitle">', escapeHtml(grade.label), " 교육과정</h1>",
      "<p>개념을 배우고 직접 만드는 9개의 수업으로 이어집니다.</p>",
      "</div></header>",
      '<div class="p2-notice" role="note">',
      '<span class="p2-notice-icon" aria-hidden="true">P2</span><div>',
      "<strong>실제 수업은 다음 단계에서 열립니다</strong>",
      "<span>지금은 커리큘럼 구조를 확인하는 P1 화면이에요. 초등 제목도 화면 구조 확인용 예시이며 확정 교안이 아닙니다. Lesson Engine과 첫 차시는 P2에서 연결됩니다.</span>",
      "</div></div>",
      '<div class="curriculum-grid" aria-label="', escapeHtml(grade.label), ' 9차시 목록">',
      lessons.map((lesson, index) => [
        '<article class="lesson-slot" aria-labelledby="lesson-',
        escapeHtml(grade.id), "-", String(index + 1), '">',
        '<div class="lesson-topline"><span class="lesson-number">',
        String(index + 1).padStart(2, "0"),
        '</span><span class="lesson-type">',
        escapeHtml(lesson[1]),
        "</span></div><h2 id=\"lesson-",
        escapeHtml(grade.id), "-", String(index + 1), '">',
        escapeHtml(lesson[0]),
        "</h2><p>P2에서 열립니다</p></article>"
      ].join("")).join(""),
      "</div>",
      "</div>",
      "</section>"
