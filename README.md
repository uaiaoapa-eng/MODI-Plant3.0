# MODI Plant 3.0 — LLM 기반 교육 플랫폼 프로토타입

edu-agent(교육용 바이브코딩 에이전트)를 기반으로, 첫 화면에서 **두 가지 모드**를
선택하도록 재구성한 프로토타입입니다.

| 경로 | 모드 | 대상 | 설명 |
|---|---|---|---|
| `/` | 랜딩 | 모두 | 교육과정 / 만들기 중 선택 |
| `/lms` | **교육과정 (LMS)** | 선생님·학교 수업 | 초·중·고 각 9차시(총 27차시) 교안. PPT 없이 화면 그대로 띄워 수업 진행 |
| `/builder` | **만들기** | 학생·자유 창작 | AI와 대화하며 같이 만들기(design 모드 전용). Web(react) 또는 MODI 블록(blockly) |

## 교육과정 (LMS) 구성

- **초등** 실과 5~6학년 · **40분** 수업 × 9차시 — 스마트 화분 프로젝트
- **중등** 정보 · **45분** 수업 × 9차시 — 스마트팜 대시보드 프로젝트
- **고등** 정보 · **50분** 수업 × 9차시 — IoT 서비스 프로젝트
- 각 학교급 공통 구조: 1~3차시 **Web 만들기** → 4~6차시 **H/W 만들기(MODI)** →
  7~9차시 **Web+H/W** (최종 목표: MODI와 연동된 웹 완성)
- 차시별 교안: 학습목표·성취기준(2022 개정 기반)·준비물·도입/전개/정리 흐름·
  교사 노트·AI 활동 프롬프트. 데이터: `static/curriculum/{elementary,middle,high}.json`
- 수업 플레이어: 수업 타이머(종료 5분 전 경고), 단계 진행 표시, 교사 노트 토글(T),
  AI 패널(A) — 수업 활동 프롬프트가 `/chat` 대화로 연결됩니다.
- ⚠ 성취기준 코드·문구는 프로토타입 초안입니다. 학교 적용 전 최신 고시본과 대조해 검수하세요.

## 프로토타입 실행

```bash
pip install fastapi uvicorn python-dotenv langfuse anthropic
uvicorn server:app --port 8000
# http://localhost:8000 접속
```

LLM 연동(.env의 Claude CLI/API 설정)이 없어도 랜딩·LMS 교안·플레이어는 동작하며,
AI 대화 기능만 서버 LLM 설정이 필요합니다. 백엔드 상세는 아래 edu-agent 문서를 참고하세요.
...
- API 연동용 계정: 아이디 `edu` / 비번 `edu-PyUTe3ZI` (NPM UI → Access Lists → `edu-agent`에서 변경)
...
```bash
curl -N -u 'edu:edu-PyUTe3ZI' https://edu-agent.luxrobo.net/chat \
...
await fetch("https://edu-agent.luxrobo.net/chat", {
  method: "POST",
  headers: {
    // Authorization: "Basic " + btoa("edu:edu-PyUTe3ZI")
    "Authorization": "Basic ZWR1OmVkdS1QeVVUZTNaSQ==",
